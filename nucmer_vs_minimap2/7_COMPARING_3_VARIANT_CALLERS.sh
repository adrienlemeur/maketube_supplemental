...
do
	...
	#-T ^/home/alemeur/SCRATCH/duplicated_region.bed
	...
	vcf2metrics.py -i ~/SCRATCH/PSEUDOVCF.vcf.gz --sample $sample --reference ~/SCRATCH/reference.vcf.gz --subtract ~/SCRATCH/dupli_region.bed --backtrack $EQUIVALENCE --add_col $sample $source "wo_duplicated" "nucmer"
	vcf2metrics.py -i ~/SCRATCH/PAF_VCF.vcf.gz --sample $sample --reference ~/SCRATCH/reference.vcf.gz --subtract ~/SCRATCH/dupli_region.bed --backtrack $EQUIVALENCE --add_col $sample $source "wo_duplicated" "minimap2"

	vcf2metrics.py -i ~/SCRATCH/PSEUDOVCF.vcf.gz --sample $sample --reference ~/SCRATCH/reference.vcf.gz --backtrack $EQUIVALENCE --add_col $sample $source "w_duplicated" "nucmer"
	vcf2metrics.py -i ~/SCRATCH/PAF_VCF.vcf.gz --sample $sample --reference ~/SCRATCH/reference.vcf.gz --backtrack $EQUIVALENCE --add_col $sample $source "w_duplicated" "minimap2"
...
do
	...
	vcf2metrics.py --sample $sample -i ~/SCRATCH/PSEUDOVCF.vcf.gz --reference ~/SCRATCH/reference.vcf.gz --add_col $sample $source "nucmer"
	vcf2metrics.py --sample $sample -i ~/SCRATCH/PAF_VCF.vcf.gz --reference ~/SCRATCH/reference.vcf.gz --add_col $sample $source "minimap2"
